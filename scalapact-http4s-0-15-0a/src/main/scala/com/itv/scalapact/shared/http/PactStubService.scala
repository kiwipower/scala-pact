...
object PactStubService {
  ...
  implicit class BlazeBuilderPimper(blazeBuilder: BlazeBuilder) {
    def withOptionalSsl(sslContextName: Option[String])(implicit sslContextMap: SslContextMap): BlazeBuilder = {
      val ssl = sslContextMap(sslContextName).fold(blazeBuilder)(
        ssl => throw new RuntimeException("Use of SSl contexts is not supported by this version of HTTP4S")
      )
  ...
  private def service(interactionManager: IInteractionManager, strictMatching: Boolean, sslContextName: Option[String])(
      implicit pactReader: IPactReader,
      pactWriter: IPactWriter
  ): HttpService =
  ...
  private def matchRequestWithResponse(
      ...
      req: Request,
      sslContextName: Option[String]
  )(implicit pactReader: IPactReader, pactWriter: IPactWriter): scalaz.concurrent.Task[Response] =
    if (isAdminCall(req)) {
      ...
      req.method.name.toUpperCase match {
        ...
        case m if m == "POST" || m == "PUT" && req.pathInfo.startsWith("/interactions") =>
          pactReader.jsonStringToPact(
            req.bodyAsText.runLog[Task, String].map(body => Option(body.mkString)).unsafePerformSync.getOrElse("")
          ) match {
            case Right(r) =>
