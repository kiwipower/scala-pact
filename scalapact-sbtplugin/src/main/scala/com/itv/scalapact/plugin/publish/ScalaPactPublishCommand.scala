...
object ScalaPactPublishCommand {
  ...
  def doPactPublish(
    ...
    if (versionToPublishAs.contains("SNAPSHOT") && !allowSnapshotPublish) {
      ...
      publishToBroker(ScalaPactHttpClient.doRequestSync, pactBrokerAddress, versionToPublishAs)(pactWriter)(
        configAndPactFiles
      ...
      configAndPactFiles.pacts.foreach { pactContract =>
        providerBrokerPublishMap.get(pactContract.provider.name).foreach { broker =>
          publishToBroker(ScalaPactHttpClient.doRequestSync, broker, versionToPublishAs)(pactWriter)(
