...
object ScalaPactVerify {
  ...
  object verifyPact {
    ...
    ): ScalaPactVerifyProviderStates = new ScalaPactVerifyProviderStates(sourceType)
    ...
    class ScalaPactVerifyRunner(
        ...
        given: Option[String],
        setupProviderState: Option[String => Boolean]
    )(implicit sslContextMap: SslContextMap) {

      def runStrictVerificationAgainst(port: Int): Unit =
        doVerification("http", "localhost", port, VerifyTargetConfig.defaultClientTimeout, strict = true)
      def runStrictVerificationAgainst(port: Int, clientTimeout: Duration): Unit =
        doVerification("http", "localhost", port, clientTimeout, strict = true)
      def runStrictVerificationAgainst(host: String, port: Int): Unit =
        doVerification("http", host, port, VerifyTargetConfig.defaultClientTimeout, strict = true)
      def runStrictVerificationAgainst(host: String, port: Int, clientTimeout: Duration): Unit =
        doVerification("http", host, port, clientTimeout, strict = true)
      def runStrictVerificationAgainst(protocol: String, host: String, port: Int): Unit =
        doVerification(protocol, host, port, VerifyTargetConfig.defaultClientTimeout, strict = true)
      def runStrictVerificationAgainst(target: VerifyTargetConfig): Unit =
        doVerification(target.protocol, target.host, target.port, target.clientTimeout, strict = true)

      def runVerificationAgainst(port: Int): Unit =
        doVerification("http", "localhost", port, VerifyTargetConfig.defaultClientTimeout, strict = false)
      def runVerificationAgainst(port: Int, clientTimeout: Duration): Unit =
        doVerification("http", "localhost", port, clientTimeout, strict = false)
      def runVerificationAgainst(host: String, port: Int): Unit =
        doVerification("http", host, port, VerifyTargetConfig.defaultClientTimeout, strict = false)
      def runVerificationAgainst(host: String, port: Int, clientTimeout: Duration): Unit =
        doVerification("http", host, port, clientTimeout, strict = false)
      def runVerificationAgainst(protocol: String, host: String, port: Int): Unit =
        doVerification(protocol, host, port, VerifyTargetConfig.defaultClientTimeout, strict = false)
      def runVerificationAgainst(protocol: String, host: String, port: Int, clientTimeout: Duration): Unit =
        doVerification(protocol, host, port, clientTimeout, strict = false)
      def runVerificationAgainst(target: VerifyTargetConfig): Unit =
